...
document.getElementById("sendOrderBtn").addEventListener("click", async () => {
  ...
  try{
    ...
    // 2) Share image + text via native Web Share API (opens WhatsApp directly with image)
    ...
    if(navigator.canShare && navigator.canShare({ files:[imgFile] })){
      btn.textContent = "Opening WhatsApp with photo…";
      try{
        ...
        // other error — fall through to deep link
      }
    }

    // 3) WhatsApp deep link — whatsapp:// opens the app directly (no browser page)
    //    After a short delay fall back to web URL for desktop browsers
    const waEncoded = encodeURIComponent(orderText);
    window.location.href = `whatsapp://send?phone=${shopPhone}&text=${waEncoded}`;
    await new Promise(r => setTimeout(r, 1400));
    if(!document.hidden){
      // Still on page = WhatsApp app not installed, open web fallback
      window.open(`https://api.whatsapp.com/send?phone=${shopPhone}&text=${waEncoded}`, "_blank");
    }

    hint.textContent = serverSent
      ? `✅ Order ${orderLabel} sent! WhatsApp also opened for your reference.`
      : `✅ Order ${orderLabel} — send the WhatsApp message to complete your order.`;
